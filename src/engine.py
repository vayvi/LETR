...
def train_one_epoch(
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        try:
            if args.LETRpost:
                outputs, origin_indices = model(
                    samples, postprocessors, targets, criterion
                )
...
def evaluate(
    ...
    if "synthetic" in args.coco_path:
        f = open(
            os.path.join(
                args.coco_path, "annotations", "lines_{}.json".format(args.dataset)
            )
        )
    else:
        f = open(
            os.path.join(
                args.coco_path, "annotations", "lines_{}2017.json".format(args.dataset)
            )
        )
    data = json.load(f)
    ...
    for samples, targets in metric_logger.log_every(data_loader, 50, header):
        ...
        if args.benchmark:
            ...
            if "data/york_processed" in args.coco_path:
                ...
                np.savez(
                    checkpoint_path.format(id_to_img[curr_img_id]),
                ...
                curr_img_id = targets[0]["image_id"].tolist()[0]
                checkpoint_path = (
                    args.output_dir + append_path + f"/{id_to_img[curr_img_id]}.npz"
                )
