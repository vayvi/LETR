    ...
    if args.resume and args.frozen_weights:
        ...
        if args.resume.startswith("https"):
            ...
            for k in checkpoint["model"]:
                if "bbox_embed" in k:
                    print(
                        "bbox_embed from OLD implementation has been replaced with lines_embed"
                    )
                    new_state_dict[
                        "lines_embed." + ".".join(k.split(".")[1:])
                    ] = checkpoint["model"][k]
                else:
                    new_state_dict[k] = checkpoint["model"][k]
            ...
            if (
                ...
                checkpoint["lr_scheduler"][
                    "step_size"
                ] = args.lr_drop  # change the lr_drop epoch
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "LETR training and evaluation script", parents=[get_args_parser()]
    )
